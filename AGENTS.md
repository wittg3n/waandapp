...
Content hash: sha256:64875454b36255ff
...
- [MUST] Keep authentication and session security on the correct side of the boundary: `apps/api` owns credential verification, sessions and preauthentication, authorization, and enforcement; `apps/user-dashboard` owns client UX/state and API orchestration but browser state is never authoritative.
...
- [AVOID] Moving app-specific UI or domain behavior into `packages/shared`, security assessment documents, or deployment files; shared packages stay framework-neutral, assessment docs record evidence, and infrastructure packages applications without owning their behavior.
...
- To change login/signup UX, auth API orchestration and guards, account-security settings, profile schemas, or onboarding: edit/read `apps/user-dashboard/src/components/auth/**`, `src/features/auth/**`, `src/features/onboarding/**`, `src/pages/auth/**`, `src/pages/settings-page.tsx`, and `src/schemas/**`; keep enforcement server-side.
...
- To change the public landing page, marketing motion/assets, testimonials, CTAs, metadata, robots, sitemap, or Next.js behavior: edit/read `apps/web/**`; preserve measured marquee-cycle and reduced-motion behavior, and let the dashboard own login/signup destinations.
- To change authorized security-assessment scope, attack-surface inventory, test evidence/status, or remediation tracking: edit/read `security/pentest/**`; implement actual safeguards in the owning app or infrastructure module.
...
- The dashboard identity module owns auth/profile UX state, API orchestration, account-security pages, and onboarding contracts; `apps/api` remains authoritative for credential verification, sessions/preauth, authorization, and mutations.
- Onboarding may consume shell-owned Iranian reference data, and dashboard product views may consume the authenticated user without taking ownership of identity.
...
- Security assessment files authorize and document bounded testing but do not implement controls or expand testing authority; fixes stay in the owning applications or infrastructure.
